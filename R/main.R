clean_cbsa_state_map = function(cbsa_state_map_file) {
  cbsa_state_map_file |>
    select(metro = cbsa23, state_fips = state) |>
...
create_acs_csvs = function(
  ...
  acs_data_final |>
    ...
    mutate(
      geo_name = str_replace(geo_name, "( Metro Area)|( Micro Area)", "")
    ) |>
    mutate(
      state_name = case_when(
        is.na(state_name) & str_ends(geo_name, ", PR") ~ "Puerto Rico",
        is.na(state_name) & str_ends(geo_name, ", CT") ~ "Connecticut",
        .default = state_name
      )
    ) |>
    select(
